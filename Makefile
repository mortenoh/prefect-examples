...
deploy: register-blocks create-blocks ## Register blocks, create instances, and deploy all flows
	...
	cd deployments/dhis2_worldbank_population_import && uv run prefect deploy --all
	...
	cd deployments/dhis2_era5_temperature_import && uv run prefect deploy --all
	cd deployments/dhis2_chirps_rainfall_import && uv run prefect deploy --all
