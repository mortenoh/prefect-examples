"""Tests for DHIS2 Open-Meteo Historical Weather Import flow."""
...
sys.modules["dhis2_openmeteo_historical_import"] = _mod
...
fetch_org_unit_historical = _mod.fetch_org_unit_historical
build_data_values = _mod.build_data_values
import_to_dhis2 = _mod.import_to_dhis2
dhis2_openmeteo_historical_import_flow = _mod.dhis2_openmeteo_historical_import_flow
...
# Minimal Open-Meteo historical response with hourly entries across 2 days.
...
def test_aggregate_weather_daily() -> None:
    results = _aggregate_weather_daily(SAMPLE_HISTORICAL_RESPONSE)
    ...
    assert results[0].temperature == pytest.approx(28.0)
    # Day 1: sum of [0.5, 0.0] = 0.5
    assert results[0].precipitation == pytest.approx(0.5)
    # Day 1: mean of [80.0, 60.0] = 70.0
    assert results[0].humidity == pytest.approx(70.0)
    # Day 1: mean of [3.5, 4.5] = 4.0
    assert results[0].wind_speed == pytest.approx(4.0)
    # Day 1: mean of [40.0, 20.0] = 30.0
    assert results[0].cloud_cover == pytest.approx(30.0)
    # Day 1: mean of [1012.0, 1010.0] = 1011.0
    assert results[0].pressure == pytest.approx(1011.0)


def test_aggregate_weather_daily_empty() -> None:
    results = _aggregate_weather_daily({"hourly": {"time": []}})
    assert results == []
...
def test_ensure_dhis2_metadata() -> None:
    ...
    assert ds["name"] == "PR: OM: WH: Historical Weather"
    assert ds["periodType"] == "Daily"
    assert ds["openFuturePeriods"] == 0
...
def test_build_data_values() -> None:
    ...
    dvs = build_data_values.fn([(ou, weather)])
...
def test_import_to_dhis2() -> None:
    ...
    assert "Historical Weather" in result.markdown
...
@patch("dhis2_openmeteo_historical_import.fetch_daily_historical")
@patch("dhis2_openmeteo_historical_import.centroid")
...
def test_flow_runs(
    ...
    state = dhis2_openmeteo_historical_import_flow(
